...
import { GetMessageDateString, MessageInfo, SyntaxHighlight } from "../../utils/utils";
...
export const MessageElement: React.FC<Props> = ({ message, hoveredMessageId, setMessageInfos, MessageInfos, _onMessageReply, setHoveredMessageId, onClickReplyMessage, onClickUserAvatar, onEditInput, onMessageDelete, onMessageEdit, _onMessageDelete }) => {
    ...
    useEffect(() => {
        if(ref.current) {
            ...
            setMessageInfos((prev) => prev.map(x => x.Message.id === messageinfo.Message.id ? messageinfo : x));
        }
    }, [ref]);

    return (
        <div className={styles.message} onMouseOver={() => onMouseHoverOver(message.id?.toString())} onMouseLeave={onMouseHoverOut} ref={ref} key={message.id}>
            {
                (() => {
                                ...
                                <p className={styles.message_reply_content} style={{
                                    width:
                                        (() => {
                                            ...
                                            if (info) {
                                                return (info.ref) ? (info.ref.clientWidth * 7 / 10) + "px" : "40vw"
                                            }
                                        })()
                                }}>{replyMsg.content}</p>
                            </div>
            ...
            <div className={styles.message_inner}>
                ...
                <div className={styles.message_user_holder}>
                    <div className={styles.message_content_holder}>
                        <p className={styles.message_username} style={{ color: useUserInfoStore.getState().getExistingUserInfo(message.author.id)?.usernameColor }}>{message.author.username}</p>
