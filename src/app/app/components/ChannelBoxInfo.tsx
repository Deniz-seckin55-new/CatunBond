...
const ChannelBoxInfo: React.FC<Props> = ({ Currents, setServerUsersDivV, setCurrents, ServerUsersDivV, onClickAddFriend, onClickFB, onClickCall }) => {
    const onClickUsers = () => {
            ...
            <div className={styles.channel_info_box_channel}>
                {(Currents.channel && !Currents.friendsdiv.visible) && (<>
                    ...
                    <div className={styles.channel_box_actions}>
                        {!Currents.vc && (<button onMouseLeave={() => onMouseLeaveTooltipElement(setCurrents)} onMouseOver={(ev) => onMouseOverTooltipElement(ev, "Call", Currents, setCurrents)} onClick={onClickCall} className={`${styles.channel_info_box_users}`}>
                        ...
                        {Currents.vc && (
                            <button onMouseLeave={() => onMouseLeaveTooltipElement(setCurrents)} onMouseOver={(ev) => onMouseOverTooltipElement(ev, "Call", Currents, setCurrents)} className={`${styles.channel_info_box_users_inactive}`}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" id="call">
                                    <path fill="none" d="M0 0h24v24H0V0z"></path>
                        ...
                        <button onMouseLeave={() => onMouseLeaveTooltipElement(setCurrents)} onMouseOver={(ev) => onMouseOverTooltipElement(ev, "Users", Currents, setCurrents)} onClick={onClickUsers} className={styles.channel_info_box_users}>
