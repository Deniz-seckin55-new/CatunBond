...
const LoadingJSX = (
    <>
        <img className={styles.loading_icon_center} src="load.svg" />
    </>
)
...
export const UserProfile: React.FC<Props> = (props) => {
    ...
    useEffect(() => {
        if (!profile.userProfile) { console.log("No userProfile."); return; }
                    ...
                    <div className={styles.user_profile_content}>
                        <div className={styles.user_profile_pad}>
                            <div className={styles.flex_rowa}>
                                <img className={styles.user_profile_avatar} src={profile.renderingUser.avatarUrl ? profile.renderingUser.avatarUrl : ''} alt={profile.renderingUser.username} width={128} height={128} />
                                <p className={styles.user_profile_name} style={{ color: profile.renderingUserInfo?.usernameColor ?? "#000D1E" }}>{profile.renderingUser.username}</p>
                            </div>
                        ...
                        <div className={styles.user_profile_bottom}>
                            <div className={styles.user_profile_pad}>
                                ...
                                <textarea className={`${styles.user_profile_notes} ${styles.up_element}`} onInput={(ev) => onNotesInput(ev.currentTarget.value)} defaultValue={initalNote}></textarea>
                            </div>
                        </div>
                    </div>
                </div>
